...
// Define interfaces for our message types and data structures
interface UIMessage {
  type: string;
  role?: string;
  text?: string;
...
if (figma.currentPage.selection.length > 0) {
  ...
  connector.connectorEnd = {
    ...
    await figma.clientStorage.setAsync(`comment-${frame.id}-role`, role);
    await updateUI();
...
async function deleteComment(id: string): Promise<void> {
  try {
    ...
    await figma.clientStorage.deleteAsync(`comment-${id}-role`);
...
async function syncComments(): Promise<void> {
  try {
    ...
    const commentFrames = figma.currentPage.findAll(node => 
      node.type === 'FRAME' && node.getPluginData('role') !== ''
    ) as FrameNode[];
...
async function updateUI(): Promise<void> {
  try {
    await syncComments();
    const message: UpdateCommentsMessage = {
      type: 'update-comments',
      comments
    };
